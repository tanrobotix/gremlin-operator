...
def cleanup_imports_and_return(imports):
    ...
    for i in imports:
        if i.strip() == "":
            continue
        if i.find("github.com/Kubedex/gremlin-operator/") != -1:
            gremlin_packages.append(i)
    ...
    with open(f, 'r') as go_file:
        ...
        for line in lines:
            if in_import_block:
                ...
                if endIdx != -1:
                    in_import_block = False
                    ordered_imports, imports_reordered = cleanup_imports_and_return(
                        imports)
